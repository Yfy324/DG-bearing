...
def get_files(root, test=False):
    ...
    dataset1 = os.listdir(os.path.join(root, m[0]))  # 'Three Baseline Conditions'
    dataset2 = os.listdir(os.path.join(root, m[2]))  # 'Seven More Outer Race Fault Conditions'
    dataset2.sort(key=lambda x: int(x.split('.')[0].split('_')[2]))
    dataset3 = os.listdir(os.path.join(root, m[3]))  # 'Seven Inner Race Fault Conditions'
    dataset3.sort(key=lambda x: int(x.split('.')[0].split('_')[2]))
    data_root1 = os.path.join(root, m[0])  # Path of Three Baseline Conditions
    data_root2 = os.path.join(root, m[2])  # Path of Seven More Outer Race Fault Conditions
    data_root3 = os.path.join(root, m[3])  # Path of Seven Inner Race Fault Conditions
...
mfpt = get_files('/home/yfy/Desktop/Dataset/MFPT/')
